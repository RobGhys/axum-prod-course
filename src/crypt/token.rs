...
pub struct Token {
    ...
    pub sign_b64u: String, // Signature - base64 encoded
}
...
impl FromStr for Token {
    ...
    fn from_str(token_str: &str) -> std::result::Result<Self, Self::Err> {
        ...
        // extracts components from token
...
fn _validate_token_sign_and_exp(
    ...
    // Check that the signature component match
...
mod tests {
    ...
    fn test_token_display_ok() -> Result<()> {
        // -- Fixtures
        let fx_token_str =
        "ZngtaWRlbnQtMDE.MjAyNi0wMS0wMVQwODozMDowMFo.blabla";

        let fx_token = Token {
            ident: "fx-ident-01".to_string(),
            exp: "2026-01-01T08:30:00Z".to_string(),
            sign_b64u: "blabla".to_string()
        };
    ...
    fn test_token_from_str_ok() -> Result<()> {
        ...
        // -- Check
        // Note: we compare the debug output. The alternative is to add PartialEq! trait to `Token` struct
    ...
    #[test]
    /// Failing test with duration < sleep time
    fn test_validate_web_token_err_expired() -> Result<()> {
        ...
        // -- Check
        // The res should be an `Error` of type `TokenExpired`
        assert!(
